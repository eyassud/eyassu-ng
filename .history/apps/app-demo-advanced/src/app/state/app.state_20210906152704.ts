    ...
    return combineLatest([choices$, countries$])
      .pipe(
        map(([choices, countries]) => {
          ctx.patchState({
            travelDecisionComponent: {
              ...state.travelDecisionComponent,
              choices: choices,
              configuration: { ...state.travelDecisionComponent.configuration, visible: true }
            },
            countryComponent: {
              ...state.countryComponent,
              choices: countries, configuration:
              { ...state.countryComponent.configuration, visible: false } }
          });
