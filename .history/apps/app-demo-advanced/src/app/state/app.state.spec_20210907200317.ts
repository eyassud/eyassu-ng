...
describe('App', () => {
  ...
  it('should be domestic travel type', async () => {
    const travelType = {"key": "Domestic", "name": "Domestic"};
    await store.dispatch(new AppActionTypes.Load()).toPromise();
    await store.dispatch(new AppActionTypes.UpdateTravelType(travelType)).toPromise();

    const app = store.selectSnapshot(state => state.app);
    console.log(JSON.stringify(app.travelTypeComponentData.selectedChoice.value));
    expect(app.travelTypeComponentData.selectedChoice).toBeTruthy();
    expect(app.travelComponentData.selectedChoice.value).toMatchSnapshot(travelType.key);
  });
});
