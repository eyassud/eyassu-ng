  ...
  ngOnInit(): void {
    ...
    this.viewModel = {
      ...
      explanationComponent$: this.store.select(AppSelectors.getExplanationComponent())
        .pipe(
          tap((textAreaData: TextAreaData) => this.getFormControl('travelType')?.setValue(textAreaData, { emitEvent: false }))
        )
    }
    ...
    this.getFormControl('explanation')?.valueChanges
      .subscribe(
        (textArea: TextArea) => asapScheduler.schedule(() => this.store.dispatch(new AppActionTypes.UpdateTextArea(textArea))));
  }
