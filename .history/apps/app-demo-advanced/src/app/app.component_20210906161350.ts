...
export class AppComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.getFormControl('destination')?.valueChanges
      .pipe(
        map((dropDown: DropDown) => { return { name: dropDown.label, key: dropDown.value }})
      )
      .subscribe(
        (country: Country) =>  asapScheduler.schedule(() => this.store.dispatch(new AppActionTypes.UpdateTravelType(country))));
